...
const injectInBuildPlugin: (outfile: string) => Bun.BunPlugin = (outfile) => ({
	...
	setup(build) {
		...
		build.onEnd(async ({ outputs }) => {
			...
			const newOutFilePath = join(cwd, build.config.outdir as string, outfile);
...
export default function createPlugin({
	...
	return {
		...
		build: {
			buildConfig: async () => ({
				files: {
					"@tailwind/bootstrap.js": await Bun.file(
						join(import.meta.dir, "..", "dist", "bootstrap.js"),
					).text(),
				},
				plugins:
						...
						: [
								{
									...
									setup(build) {
										...
										build.finally("html", ({ contents }) => {
											return {
												contents: rewriter.transform(contents as string),
											};
										});
									},
								},
